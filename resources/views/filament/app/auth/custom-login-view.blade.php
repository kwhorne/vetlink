<div
    ...
    x-init="startAuto()"
    class="flex flex-col items-center justify-center min-h-screen text-center bg-[#f8fbfd] relative overflow-hidden border-r border-gray-200"
>
    <!-- Fiksna slika -->
    ...
    <template x-for="(slide, index) in slides" :key="index">
        ...
        >
            <h2 class="text-sky-500 font-semibold text-base mb-2" x-text="slide.title"></h2>
            <p class="text-gray-700 font-medium text-sm mb-3 max-w-md" x-text="slide.text"></p>
            <template x-if="slide.link">
                <a :href="slide.link" class="text-sky-500 font-semibold text-sm hover:underline" x-text="slide.linkLabel"></a>
            </template>
        </div>
    </template>
    ...
    <div class="absolute bottom-10 flex items-center justify-center space-x-6">
        ...
        <template x-for="(slide, index) in slides" :key="'dot-' + index">
            <button
                @click="active = index"
                :class="active === index ? 'bg-sky-500 w-4' : 'bg-gray-300 w-2'"
